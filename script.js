...
function openMega(menuKey) {
  ...
  // Set first item ID as default
  const defaultKey = data.leftCols[0].items[0].id;

  // Always load only default images/text
  renderMegaRight(menuKey, defaultKey);

  // ❌ No click event for left items (so images don't change)
  megaLeft.querySelectorAll('.left-item').forEach(el => {
    el.style.cursor = "default"; // disable pointer cursor
  });
...
topLinks.forEach(t => {
  t.addEventListener('click', e => {
    ...
    if (activeTop === key) {
      // Close menu if clicking same link again
      activeTop = null;
...
/* -------- FAVORITES SLIDER DOTS -------- */
// const favCards = document.querySelectorAll('.favorites-cards .fav-card');
// const favCarousel = document.querySelector('.favorites-cards');
// const favDotsContainer = document.querySelector('.fav-dots');
// const favDots = favDotsContainer.querySelectorAll('.dot');

// function updateFavoritesSlider(index) {
//   const cardWidth = favCards[0].offsetWidth + 20;
//   const offset = -index * cardWidth;
//   favCarousel.style.transform = `translateX(${offset}px)`;

//   favDots.forEach((dot, i) => {
...
//     updateFavoritesSlider(i);
//   });
// });

// updateFavoritesSlider(0);

// function initSlider(sliderSelector, prevBtnId, nextBtnId) {
...
// initSlider('.collection-slider', 'collection-prev', 'collection-next');
// initSlider('.favourite-slider', 'fav-prev', 'fav-next');
// initSlider('.product-slider', 'product-prev', 'product-next');
...
function showInstaSlide(index) {
  ...
  instaImages.forEach((img, i) => {
    // Always show first 5 images only
...
// Add click event to Clifton dots
...
let currentIndex = 0; // Start from first dot
const totalSlides = dots.length; // Number of dots
...
nextBtn.addEventListener('click', () => {
  currentIndex = (currentIndex + 1) % totalSlides; // Move forward
  updateDots();
});

prevBtn.addEventListener('click', () => {
  currentIndex = (currentIndex - 1 + totalSlides) % totalSlides; // Move backward
  updateDots();
});

// Optional: Allow clicking dots to jump to that slide
dots.forEach((dot, index) => {
  dot.addEventListener('click', () => {
    currentIndex = index;
    updateDots();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  buyButtons.forEach(button => {
    button.addEventListener("click", (e) => {
      ...
      // Find the parent fav-card
      const favCard = button.closest(".fav-card");

      // First, remove clicked class from all other cards
  ...
  // Optional: click outside to close the detail card
